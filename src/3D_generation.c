...
/* Define the global variable gen */
struct generation gen;

/* Implementation of your functions using gen */
...
{
    if (gen.texture_number < 0 || gen.texture_number >= NUM_TEXTURES) {
        fprintf(stderr, "Invalid texture number: %d\n", gen.texture_number);
        exit(EXIT_FAILURE);
    }

    if (wall_textures[gen.texture_number].texture_buffer == NULL) {
        fprintf(stderr, "Texture buffer is NULL for texture number: %d\n", gen.texture_number);
        exit(EXIT_FAILURE);
    }
    ...
    if (gen.texture_offset_y < 0 || gen.texture_offset_y >= TEXTURE_HEIGHT) {
        fprintf(stderr, "Invalid texture offset Y: %d\n", gen.texture_offset_y);
        exit(EXIT_FAILURE);
    }

    if (gen.texture_offset_x < 0 || gen.texture_offset_x >= TEXTURE_WIDTH) {
        fprintf(stderr, "Invalid texture offset X: %d\n", gen.texture_offset_x);
        exit(EXIT_FAILURE);
    }
...
void generate3DProjection(void)
{
    ...
    for (i = 0; i < NUM_RAYS; i++)
    {
        ...
        for (y = gen.wall_bottom_pixel; y < WINDOW_HEIGHT; y++)
            r.color_buffer[(WINDOW_WIDTH * y) + i] = 0xFFD3D3D3;
    }
}
